...
//这个名称与OrderApplication.java中@EnableJpaAuditing(auditorAwareRef = "auditAwareImpl")的配置完全对应
public class AuditAwareImpl implements AuditorAware<String> {//作用是返回当前操作用户的标识（这里是用户名）
// AuditorAware<String> 是 Spring Data JPA 提供的一个接口，专门用于获取当前操作数据的用户标识
// 泛型 <String> 表示这个用户标识的数据类型是字符串（比如用户名、用户 ID 字符串等）。
    
    @Override
    public Optional<String> getCurrentAuditor() {// 因为AuditorAware接口的getCurrentAuditor()方法强制要求返回Optional<T>类型，这是 JPA 的规范。
            return Optional.ofNullable(//将可能为 null 的用户名包装成 Optional 对象，避免空指针异常
